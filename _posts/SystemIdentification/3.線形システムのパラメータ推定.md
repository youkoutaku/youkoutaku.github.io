...
$$y_t=x_t+v_t$$
>一般に，観測雑音 $v_t$ はシステムの入力信号 $u_t$ とは，無相関である．すなわち，
>$$E[u_tv_t]=0$$
>ただし，$E[・]$は期待値を意味する．
...
$$\begin{equation} \frac{\partial J}{\partial \theta}&= \frac{\partial}{\partial \theta}\left( \sum_{t=1}^{N} \rho^{N-t}(y_t-z_t^T\mathbf{\theta})^2 \right)\\&=-\sum_{t=1}^{N}\rho^{N-t}2z_t(y_t-z_t^T\mathbf{\theta})\\&=-2\sum_{t=1}^{N}\rho^{N-t}z_ty_t+2\sum_{t=1}^{N}\rho^{N-t}z_tz_t^T\theta = 0
\end{equation}$$
...
ここで，忘却係数$\rho=1$，すなわち**過去のデータを忘却しない**場合について考察する．
 $\hat\theta$ の式に $y_t=z_t^T\theta+r_t$を代入すると，$$\begin{equation}
...
$$\begin{equation} plim_{N\to\infty} &= \left(plim_{N-\infty}\frac{1}{N}\sum_{t=1}^{N} m_tz_t^T \right)^{-1}\left( plim_{N\to\infty} \frac{1}{N}\sum_{t=1}^{N}m_t\{ z_t^T\theta+r_t \} \right) 
\\&=\theta+\left(plim_{N-\infty}\frac{1}{N}\sum_{t=1}^{N} m_tz_t^T \right)^{-1}\left( plim_{N\to\infty} \frac{1}{N}\sum_{t=1}^{N}m_tr_t\right)  \end{equation}$$
が得られる．
...
Step a: If $\hat a_{i,N-1}(i=1,2,\dots,n)$  stable then
				$\bar a_{i,N-1}=\hat a_{i,N-1}(i=1,2,\dots,n)$
				Stop
			Else
				$\bar a_{i,N-1}=\bar a_{i,N-2}+0.5*(\hat a_{i,N-1}-\bar a_{i,N-2})$
			Endif
Step b: If $\bar a_{i,N-1}(i=1,2,\dots,n)$  stable then
				Stop
			Else
...
# 3.6 逐次最小二乗法の導出
[[3.線形システムのパラメータ推定#3.2 **最小二乗法**の導出 least squares method|最小二乗法 ]]による二乗誤差評価$\partial J/\partial{\theta}=0$のとき，最小二乗法推定値 $\widehat{\theta}$ ）により，
...
$$\begin{equation}\widehat\theta_N&=\left(\sum_{t=1}^N\rho_N^{N-t}z_t z_t^T\right)^{-1}\sum_{t=1}^N\rho_N^{N-t}z_t y_t
...
$$\begin{equation}\widehat\theta_N&=P_N\left[ \rho_N P_{N-1}^{-1}\widehat\theta_{N-1}+z_Ny_N \right]
...
$$\begin{equation}P_Nz_N&=L_N
\\&=\frac{1}{\rho_N}\left[ P_{N-1} - \frac{P_{N-1}z_Nz_N^TP_{N-1}}{\rho_N+z_N^TP_{N-1}z_N} \right]z_N\\&=\frac{P_{N-1}z_N}{\rho_N+z_N^TP_{N-1}z_N}
\end{equation}$$
...
>$$\alpha_1x_1+\dots+\alpha_kx_k=0\implies [x_1\;\dots\;x_k]\begin{bmatrix}
...
**左零空間**：$$\mathcal N(X^T)=\{n\in\mathbb R^N| X^Tn=0\}$$
$$dimN(X^T)=N-r$$
**列空間と左零空間が直交**している．
...
$$\begin{equation} (1)&&  AGA&=A\\ (2)&& (AG)^H&=AG\\ (3)&& GAG &= G\\ (4)&& (GA)^H&=GA    \end{equation}$$
