...
1. 制御対象のモデルを作成する。(モデルベース)
	1. 現実ではほとんど非線形モデルである。モデルが必要不可欠である。モデルが不正解だと、予測した最適解が無意識なものになる。
2. 目的にあった評価関数(アルゴリズム)と拘束条件(物理的制限)を設定する。
	1. 良い性能を得るために、適切な評価関数を設計する。
	2. 制御対象の物理的制限によって拘束条件を与える
3. 最適解が満たすべき条件を得る。
	1. 解が簡単に求められなくて、条件を満たすように解の候補を繰り返し修正していく。その繰り返しは多くの計算が必要であり、計算時間が制御サンプル時間より多くなる場合がある。そのために実用化が難しい。
	2. 近年、計算機の性能と数値解法の進歩により、実時間最適化(real-time optimization)が実現可能になる。
4. 数値計算によって最適解を見つける。
